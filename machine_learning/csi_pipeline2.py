...
class CSIPipeline:
            ...
            while self.running:
                if ser.in_waiting:
                    ...
                    if 'CSI_DATA' in line:
                        ...
                        try:
                            # Extract the data part which is a string representation of an array
                            # Find the position of the last quote mark from the end
                            if '"[' in line and ']"' in line:
                                # Extract the array part using regular expression
                                array_match = re.search(r'"(\[.*?\])"$', line)
                                
                                if array_match:
                                    # Get the array string without the surrounding quotes
                                    ...
                                    if len(csi_raw_data) in [128, 256, 384]:
                                        ...
                                        print(f"Invalid data length: {len(csi_raw_data)}")
                                else:
                                    print("Could not extract array from line")
                            else:
                                print("CSI_DATA line doesn't contain array data in expected format")
                                
                        except Exception as e:
                            print(f"Error processing CSI data: {e}")
                            print(f"Problematic line: {line[:50]}... (truncated)")
                else:
    ...
    def preprocess_data(self, samples):
        ...
        # Reshape for LSTM model if needed (check your model's expected input shape)
        # If your model expects input shape: (samples, time_steps, features)
        # You might need to reshape: X_normalized = X_normalized.reshape(X_normalized.shape[0], X_normalized.shape[1], 1)
